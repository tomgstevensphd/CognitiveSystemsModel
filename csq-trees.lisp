...
;; PCSYM=  ("KIND" "KIND vs UKIND" CS2-1-1-99 NIL NIL :PC ("KIND" "UKIND" 1 NIL) :POLE1 "KIND" :POLE2 "UKIND" :BESTPOLE 1 :BIPATHS ((POLE1 NIL MOTHER NIL) (POLE1 NIL BEST-M-FRIEND NIL) (POLE2 NIL FATHER NIL)) :CSVAL "0.917" :CSRANK "2")
;;ELMSYM= ("BEST-M-FRIEND" "best-m-friend" ELM4-1-1-99 NIL "JOE" :BIPATHS (((BEST-M-FRIEND NIL (FEMA (POLE2) NIL)))))
...
(defun make-pc-tree (pc-list &key   )
  ...
  Note: Each symbol (pcsym) is includes (name-str poles location (cs2-1-1-99)  with keys :pole1 :pole2 :bestpole :csval :csrank. The nest/tree level is determined by the csval and the sort order within the level by the csrank. 
    SUBTREES (next nest level):  If a pc has a :bipaths subkey :sub, then the sub pcs are placed in a list to the right of the pc."
  (let
      ((x)
       )
