...
#|        (setf target-dim-n1 (list-length subdimspecs1)
      ...
      (cond
       ...
       (t
         ...
         ;;(BREAK "AFTER make-node-dimsym-tree RECURSE")
         ;;VVV
         (when new-subsyms
           (setf all-node-tree-syms (append all-node-tree-syms (list new-subsyms))))
         ;;end t,cond
...
(defun find-art-class-rootsym (sym &optional dimspecs-list 
  ...
  "In U-CS-symbol-trees.lisp, RETURNS (values class-rootsym symvals) (eg. X or WUP)   Second in symvals is DIM-SPECS-LIST NOT DIMS-LIST"
  (let*
      ((symvals)
       (dims )
       (rootstr)
...
(defun find-class-dimspecs (sym)
  "In U-CS-symbol-trees.lisp,  Finds class-dimspecs--which apply to all instances in the class. INPUT= any sym in the class. RETURNS (values class-dimspecs rootsym). Finds/uses class-rootsym."
  (let
      ((class-dimspecs)
       )
...
(defun dimlist-length (dimlist &key n-ints-first-p  (dim-letters *art-index-syms) 
  ...
  "In U-CS-symbol-trees.lisp, RETURNS (values n-dims  n-items sublist-ns n-ints) for a dimlist (eg. (1 2 3 TO 4 5 6)). SUBLIST-NS is a list of  n-subitems key n-subitems, etc. NOTE: N-DIMS at end is total n-dims incl letters. N-INTS-FIRST-P puts n-ints first and n-dims last in values returned."
  (let
      ((n-dims 0)
       (n-ints 0)
...
(defun find-super-subord-index (sym &key (superord-subord-lists '((Top M)(M F)(F L)(L I)(I B))) return-subord-index-p)
  "In U-CS-symbol-trees.lisp. RETURNS If :return-subord-index-p returns subord-sym otherwise superord-sym."
  (let
      ((superord-sym)
       (subord-sym)
       (return-sym)
