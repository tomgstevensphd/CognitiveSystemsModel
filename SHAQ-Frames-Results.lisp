...
;;
;;ddd
(defun shaq-problems-callback (item interface)
     "In Frames-results"
...
;;
;;ddd
(defun make-shaq-results-text (&key (data-list *CSQ-DATA-LIST) (set-global-results-text-vars-p  t))
  "in frames-results.  input *CSQ-DATA-LIST or data-list of same format. returns complete shaq results text. if set-global-results-text-vars-p, then creates global vars *scales-results-text and *scales-ss-quests-results-text. note: (make-all-questions-text) must be run first."
  ;;define vars and find intro text data
  (let*
      ((credit-class-instructor "")
       (date-string  "")
       (id-data-list  (get-scale-datalist "sID"))
       ;;(third (get-key-value-in-nested-lists '(("sid" 1)) *CSQ-DATA-LIST :return-list-p t))
...
")
    ...
    (multiple-value-setq (date-string  date  time)
     (my-get-date-time))
    (setf *CSQ-DATA-LIST (append *CSQ-DATA-LIST (list :date date :time time)))
   ...
   ;;1.1 find intro data done above 
    ...
    (setf  intro-results-text (make-shaq-results-intro-text date-string  name user-id
                   sex   age-string   emailaddress nation  zipcode   
   ...
   (multiple-value-setq (priority-button-names other-button-names)
    ...
    ;;SET GLOBAL VARIABLES?
      ...
      ;;STEP 7: ASK USER IF WANTS TO SAVE LARGEST (& *CSQ-DATA-LIST)
      ;;           to a file.
...
;;
;;;ddd
(defun make-shaq-results-intro-text ( date-string  name user-id
   ...
   DATE: ~A
        Name: ~A          User ID: ~A     Sex: ~A     Age: ~A      Email: ~A 
...
;;
;;ddd
(defun process-scales-results (&key (q-datalist *CSQ-DATA-LIST) 
...
;; (modify based upon above function doing data analysis and adding to *CSQ-DATA-LIST
;;
;;ddd
(defun process-scale-results (scalevar   &key  is-subscale-p scaledata-list 
                                           super-scale (data-list *CSQ-DATA-LIST)
                                           (all-scales-datalist *shaq-scaledata-list) )
  "in frames-results,  1.given scale-name and question-data-list (in *CSQ-DATA-LIST format) in data-list ,2.calcs scale data, 3. adds to *CSQ-DATA-LIST, 4. writes data in formatted results. returns (values scale-text button-keylist-list helplink-name-list helplink-url-list helplink-description-list help-priority scale-quests). appends *CSQ-DATA-LIST with (list scale-name  num-q-data-lists scale-rel-score scale-mean-score scale-max-score scale-sd scale-variance) and adds same items to slots in the scale'-inst. note: last item of  button-keylist-list is the user-help-priority."
 ;; (slot-value *st1higherself-inst  'mean-score)
  (let*
   ...
   (when (and help-priority scale-user-mean scale-rel-score)
...
"
    ...
    (setf button-keylist-list (append button-keylist-list (list user-help-priority)))
      ...
      ;; 1.Finds question data in *CSQ-DATA-LIST
  ;;2.calc scale data, 3. add to *CSQ-DATA-LIST, 4. use here in results.
...
;;
;;ddd
#|(defun make-subscale&quests-text (scale &key (datalist *CSQ-DATA-LIST))
...
;;
;;ddd
(defun get-scale-quests-text (qvarlist &key ;;scale-name subscale-name
...
;;
;;ddd
(defun make-all-questions-text (&key (all-questsdata-list *CSQ-DATA-LIST))
       "In Frames-results, RETURNS all-text-questsyms-list for all questions completed in order from *CSQ-DATA-LIST.  Each item in the list is a symbol of the question qvarT set to a '(qvar quest-text qvar scale-name) -- later incl subscale?)"
...
(defun make-multi-selection-quest-text (multi-qvar &key quest-num multi-qvar-scaledata-list) 
 "In Frames-results, RETURNS (values multi-sel-quest-text help-info help-links). Finds multi-qvar-scaledata-list if not given."
  (let*
      ((multi-sel-quest-text "")
        (multi-selection-qvarlist (get-multi-selection-quest-var-values multi-qvar))        
     ...
     ;;kkkk
       ...
       ;;  (afout 'out (format nil "MULTI-SELECTION-QVARLIST= ~A~%" multi-selection-qvarlist))
       ;; (q-text-sym (getf multi-selection-qvarlist :primary-qvar-sym))
       ;; (qvar (my-make-symbol  (getf multi-selection-qvarlist :primary-qvar-sym)))
       ;; (label (getf multi-selection-qvarlist :primary-qvar-label))
       ;; q-name (getf multi-selection-qvarlist  :primary-qvar-sym
       (title-text (getf multi-selection-qvarlist :primary-title-text))
       (instruction-text  (getf multi-selection-qvarlist :primary-instr-text))
       (question-text-list  (getf multi-selection-qvarlist :quest-text-list))
       ;; (qvar-name-list  (getf multi-selection-qvarlist    :ans-name-list))
       ;;(ans-text-list  (getf multi-selection-qvarlist  :ans-text-list))
       ;;(quest-data-list  (getf multi-selection-qvarlist :ans-data-list))
       (num-answers  (getf multi-selection-qvarlist  :num-answers))
       (q-num quest-num)
       (q-num-text "")
       ;;not needed     ans-xdata-list  (getf *multi-selection-qvarlist :ans-xdata-list))
       (help-info (getf multi-selection-qvarlist  :help-info))
        (helplinks-list (getf multi-selection-qvarlist  :help-links))
        (button-keylist-list)
        (helplink-name-list) 
        (helplink-url-list)
        (helplink-description-list)
...
;;
;;ddd
(defun make-graph-string (score  &key (substring "|") (intro-string "") (length 100) (begin 0) (end 1.0) (poststring ""))
...
;;
;;ddd
(defun make-no-scale-text (&key (csq-data-list *CSQ-DATA-LIST))
