;;******************************** CSQ.lisp *************************
;;
;; MAIN FILE FOR COGNITIVE SYSTEMS QUESTIONNAIRE (CSQ) MANAGEMENT FUNCTIONS
;;
;;Adapted from  U-SHAQ-Management.lisp ***********************
;;
...
;;CAUSES WINDOW TO POP UP INSTEAD OF DEBUGGER FOR ALL LW
;;(dbg:log-bug-form "DEBUG in CSQ.lisp" :log-file "CSQ-Deliver LOG.lisp" :message-stream T)

(defparameter *CSQ-initialized NIL "Set to T when function run to initialize")
(defparameter *CSQ-files-loaded NIL "Value changed after load-CSQ-files run")
(defparameter *1-MP-PROCESS-m NIL)

;;TO RUN CSQ
;; 1-USE (GOCSQ) FOR INTERPRETED VERSION (Uses CSQ-config to load)
...
;; 1. run GOCSQ (then exit) [or if loads all, CSQ (loads everything)]
;; 2. compile buffer  CSQ-data-list-eg1.lisp  (or other that sets *shaq-data-list and *shaq-scaledata-list to test data.
...
;;; GOCSQ -- (FOR LISP INTERPRETED VERSION)
;; 
;;ddd GGG
(defun goCSQ (&key (run-qvar-list *run-qvar-list) (test-scale-list *test-scale-list )                     (run-CSQ-intros-p *run-CSQ-intros-p))
  "In CSQ, RUN-QVAR-LIST is a list of qvars/questions to run that EXCLUDES running all other questions. TEST-SCALE-LIST is a list of test scales."
  (setf out nil)
  ...
  (unless  (boundp '*CogSys-loaded )
    (load  "C:\\DROPBOX\\LISP PROJECTS TS\\CogSys-Model\\CSQ-config.lisp"))
  ;;(unless *shaq-data-list        (load  "C:\\DROPBOX\\LISP PROJECTS TS\\SHAQ\\config-vars.lisp"))
  (unless *CSQ-initialized
    (csq-initialize))
  (setf *CSQ-data-list '(:CSQ-data-list)
        *CSQ-scaledata-list '(:CSQ-scaledata-list)
        *run-CSQ-intros-p run-CSQ-intros-p)
  (CSQ-process-manager    ;;  *CSQ-post-sym-list *CSQ-scale-sym-list  
...
;;
;;ddd
(defun runCSQ (&key (run-qvar-list *run-qvar-list) (test-scale-list *test-scale-list )                     (runCSQ-intros-p *run-CSQ-intros-p))
  "In CSQ,  RUN-QVAR-LIST is a list of qvars/questions to run that EXCLUDES running all other questions. TEST-SCALE-LIST is a list of test scales.
...
;; (unless *shaq-initialized   (CSQ-initialize))
  (setf *CSQ-data-list '(:CSQ-data-list)
        *CSQ-scaledata-list '(:CSQ-scaledata-list)
        *run-CSQ-intros-p run-CSQ-intros-p)
...
;;MAKE THE CLASS INSTANCES
  ...
  ;;(make-CSQ-scale-instances) instances made by defparameters when the inst file loaded
...
;;  
;;ddd
(defun CSQ-process-manager (&key (question-sym-list *All-PC-elementQs)
                                   run-qvar-list test-scale-list  (run-CSQ-intros-p T)
                                   (append-to-scale-cat-list '(HQ)))
                                                   ;;no args for Deliver?  text height)
  "In U-CSQ-management.lisp, manages all processes makes the basic specific-instance elements.  RUN-QVAR-LIST is a list of qvars/questions to run that EXCLUDES running all other questions.  =  question-sym-list "
  (let (  ;;not used?? (manager-mailbox (mp:ensure-process-mailbox))
        (text "Cognitive Beliefs and Values Questionnaire")
        (height 100)
        ) 

    ;;INITIALIZE CSQ IF NOT DONE
  ;;MOVED  (unless (and (boundp '*deliver-version-p) (null *deliver-version-p)) (initialize-CSQ))
...
;;
;;CSQ--MANAGER 
;;  Main process to manage CSQ 
;;
;;ddd
(defun CSQ-manager (question-sym-list  run-qvar-list test-scale-list run-CSQ-intros-p append-to-scale-cat-list &key not-send-CSQdat-p)
  "In CSQ, Called by CSQ main process.  Code for management of entire CSQ process. append-to-scale-cat-list adds superclass scale categories get scale-lists from."
  (let 
      ((userid)
       (user-CSQdata-list)
    ...
    ;;     (this is redundant to setting in CSQ-process-manager)
    ...
    ; STEP 1: RUN THE INTRO FRAMES 
    ;;MUST RUN before assemble rest of data
...
#|      (setf intro-selected-scalecats (remove-duplicates
   ...
   ;;STEP 2: RUN  THE CSQ QUESTIONS

   ;;STEP 2A: RUN FIND PC-ELEMENTS QUESTIONS
   
   ;;SSSS START HERE -- IN CSQ-MANAGER, RUNNING CSQ QUESTS
   (make-pc-elements-frames)


   ;;STEP 2B: RUN THE COMPARE PC-ELEMENTS QUESTIONS



    
    ;;STEP 2C: RUN SCALES,QUESTIONS and and GATHER DATA into *CSQ-data-list
    ;;
 ...
 ;;KKK NO BIO3EDUC BIO4JOB BI07LANG BIO1ETHN BIORELAF FRAME-FAMILY-INFO (run-frame-family-info)
    ...
    ;;(afout 'out (format nil "In CSQ-MANAGER, test-scale-list= ~A~%" test-scale-list))
...
(defun check-for-frame-visible ()
  "In U-CSQ-management.lisp, used by mp:current-process-pause to keep it open for input from a poke"
...
(defparameter  *OPEN-DOS-DEBUG-WINDOW NIL)

(defun my-debugger-hook (condition old-debugger-hook)
  "In U-CSQ-management, (copied basic from LW-examples\deliver\..."
  ...
  (path 
    ...
    (capi:display-message 
     "
    Log of  CSQer that occured  ~a ~%    written to: ~a   

     PLEASE WAIT:  CSQ IS BUSY COMPUTING!
...
#|(defun make-question-selection-list (matching-scales-lists &key (CSQ-data-list *CSQ-data-list))
  "In Frames-CogTest-intros.  1. Takes the (SCALESSEL :MULTI \"scalessel\" \"scales-selected\" ...) list created by select-CogTest-scales and user-chosen in frame to create a single list of  selected scales to be run by function (process-selected-question-list (selected-question-list). matching-scales-lists should be a list of lists with first item matching the multi-sel answer in SCALESSEL, and the second item either a scale-name or a scale category name eg. OUTCOME.  RETURNS list of scale-names and category names. to pass to a function to process category names and/or scale-names and run the questions. (Put in separate function to allow intermediate operations)."
 (let
     ;;should SCALESSEL be a string--NO.
     ((select-CogTest-scales-results (get-qvarlist-in-CogTestdatalist 'SCALESSEL))
      )
...
;;TEST
;;  (select-CogTest-scales)
;; (defun goq4 ()  (progn (setf out nil) (setf *qfr4-inst (select-CogTest-scales))))
;;  (goq4)
;; 2014-07
#|(
...
(defun process-multi-selection-results (qvar &optional matching-items-list)
  "In U-CSQ-management--move??  1. Finds the results list for qvar in *CSQ-data-list, 2. RETURNS (values selected-lists-with-cdr-match-list matched-items-list) (selected from matched items in matching-items-list--if supplied-otherwise, matched-items-list = nil.= matching-items-list eg= '((\"answer 1\" \"scale-name1\")...)."
  (let*
      ((qvar-list (get-qvarlist-in-CSQdatalist qvar))
...
;;TEST (must run (select-CSQ-scales) first to generate *CSQ-data-list
...
(defun make-scales-frames  (scalecat-list &key scale-cat-list reverse-subclass-order-p
  ...
  "In U-CSQ-management, INPUT=list of scales to make question-frames for every question in every scale in the list.  Both Single and Multi-selection questions can be included in the qvar lists in the :scale-questions slot of each scale. reverse-subclass-order-p causes order of cat-scales to be opposite those in class definitions."
...
;;MMM IN MAKE-SCALE-FRAMES
  (let*
    ...
    ;;(afout 'out (format nil "BBB question-list= ~A~% run-qvar-list= ~A%" ques
    (loop
     ...
     ;;SSSS START HERE IN MAKE-SCALE-FRAMES -- MAKE-QUEST
...
;;make-scale-frames TESTED with (goCSQ) and works well, ROBUST using previous frames button.
