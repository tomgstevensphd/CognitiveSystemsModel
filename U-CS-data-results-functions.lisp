...
;;Replaced all SHAQ with CSQ
;;
;; Functions for manipulating CSQ RESULTS DATA
;;  (after questions were answered, many on the *CSQ-data-list )
;;
...
;;
;; A. FUNCTIONS USING *CSQ-DATA-LIST
;; 1.  GET-QVARLIST-IN-CSQDATALIST 
;; INPUT: Searches *CSQ-data-list for QVAR  
;; OUTPUT: QVARLIST*

;; 2.  GET-MULTI-ANSWER-SELECTED-ITEMS
;;  INPUT:  (qvar &key (all-data-list *CSQ-data-list))  (a multi-selection qvar --generally a category such as BIO. Selected items have a value of 1 (vs 0 for non-selected). 
...
;;GET-QVARLIST-IN-CSQDATALIST
;;
;;ddd
(defun get-qvarlist-in-CSQdatalist (qvar &key return-second-p  (data-list *CSQ-data-list))
  ...
  (let
    ...
    ;;end let, get-qvarlist-in-CSQdatalist
    ))
;;TEST
...
;;TEST2
;;works  (get-qvarlist-in-CSQdatalist "utype" ) --works   :CSQ-data-list data-list)
...
;;RETURNS (("twanttho" (:SCALES (HQ))) ("tprevCSQ" (:SCALES (PREVIOUS-USER))) ("wantspq" (:SCALES (SPECIFIC-QUESTS))) ("u-none" (:SCALES NIL)))    "UTYPE"  (:SCALES HQ PREVIOUS-USER SPECIFIC-QUESTS)    (("twanttho" "1" 1 T 1 2 (:XDATA :SCALES (HQ))) ("tprevCSQ" "6" 1 T 1 2 (:XDATA :SCALES (PREVIOUS-USER))) ("wantspq" "7" 1 T 1 2 (:XDATA :SCALES (SPECIFIC-QUESTS))) ("u-none" "14" 1 T 1 2 (:XDATA :SCALES NIL)))
...
;;
;;ddd
(defun get-all-multi-sel-items-data (&key (scaledata-list *CSQ-scaledata-list))
...
;;
;;ddd
(defun get-multi-qvar-scaledata-list (multi-qvar &key (CSQ-scaledata-list *CSQ-scaledata-list) all-multi-qvar-scaledata-lists)
...
;;
;;ddd
(defun get-multi-answer-selected-items (qvar &key (all-data-list *CSQ-data-list) msq-datalist)
...
;;TEST
;;  (get-selected-qvars&xdata '("UTYPE" :MULTI "UserType" "User characteristics/assessment choices" 2 ("twanttho" "1" 1 T 1 2 (:XDATA :SCALES (HQ))) ("tknowmor" "2" 1 NIL 0 2 (:XDATA :SCALES (HQ))) ("twanthel" "3" 1 NIL 0 2 (:XDATA :SCALES (HQ))) ("twantspe" "4" 1 NIL 0 2 (:XDATA :SCALES NIL)) ("texperie" "5" 1 NIL 0 2 (:XDATA :SCALES NIL)) ("tprevCSQ" "6" 1 T 1 2 (:XDATA :SCALES (PREVIOUS-USER))) ("wantspq" "7" 1 T 1 2 (:XDATA :SCALES (SPECIFIC-QUESTS))) ("tu100stu" "8" 1 NIL 0 2 (:XDATA :SCALES (HQ ACAD-LEARNING))) ("tcsulbst" "9" 1 NIL 0 2 (:XDATA :SCALES (ACAD-LEARNING))) ("tcolstu" "10" 1 NIL 0 2 (:XDATA :SCALES (ACAD-LEARNING))) ("totherst" "11" 1 NIL 0 2 (:XDATA :SCALES (ACAD-LEARNING))) ("tressub" "12" 1 NIL 0 2 (:XDATA :SCALES NIL)) ("tcolfaca" "13" 1 NIL 0 2 (:XDATA :SCALES NIL)) ("u-none" "14" 1 T 1 2 (:XDATA :SCALES NIL)))  :xdata-key-list '(:SCALES)  :return-one-key-values ':scales)
;;; WORKS, = (("twanttho" (:SCALES (HQ))) ("tprevCSQ" (:SCALES (PREVIOUS-USER))) ("wantspq" (:SCALES (SPECIFIC-QUESTS))) ("u-none" (:SCALES NIL)))     "UTYPE"    (:SCALES HQ PREVIOUS-USER SPECIFIC-QUESTS)      (("twanttho" "1" 1 T 1 2 (:XDATA :SCALES (HQ))) ("tprevCSQ" "6" 1 T 1 2 (:XDATA :SCALES (PREVIOUS-USER))) ("wantspq" "7" 1 T 1 2 (:XDATA :SCALES (SPECIFIC-QUESTS))) ("u-none" "14" 1 T 1 2 (:XDATA :SCALES NIL)))
...
;;
;;ddd
#|(defun pretty-print-CSQ-data-list (&key (CSQ-data-list *CSQ-data-list ))
  "In U-data-results, pretty-prints  *CSQ-data-list or other specified data-list instead. Can just use regular PPRINT FUNCTION TOO."
  (let
      ((pretty-data-string (format nil "(:CSQ-DATA-LIST~%"))
       (new-item-num -99)
       )
    (loop
     for item in CSQ-data-list
     with scale-name
     with scale-list
     with new-scale-p
     do
    ...
    pretty-data-string
    ;;end let, pretty-print-CSQ-data-list
    ))|#
;;TEST
...
(defun calc-scale&subscale-scores (&key (data-list *CSQ-data-list) (omit-list '(:CSQ-data-list)))
  ...
  (let
    ...
    (loop
     ...
     ;;calcs scale scores and appends to *CSQ-scaledata-list
      ...
      (process-scaledata-p
       ...
       (unless  (or (null scale-name)(string-equal scale-name "no-scale"))
         ...
         (loop
          ...
          ;;calcs scale scores and appends to *CSQ-data-list
...
(defun calc-scale-data (scale-name quest-data-lists &key is-subscale-p (scoring-formula '+)) ;;alternative  append-data-lists-p (data-list *CSQ-data-list) (scaledata-list *CSQ-scaledata-list))
  ...
  (cond
   ...
   ;;SSS JUST ADDED THIS-- TEST WITH GOCSQ
   (scale-name  
    ...
    ;;PUT DATA IN SCALE-INSTANCE AND *CSQ-DATA-LIST and *CSQ-scaledata-list
...
;;works, appends    :SCALEDATA INTSS7COLLAB (INTSS7COLLAB 7 0.143 1 7 49 0.0 0))  to *CSQ-data-list



#|  replace with process built into results manager???
(defun next-CSQ-data-function (&key (data-list *CSQ-data-list))
  ...
  (let
    ...
    ;;end let, next-CSQ-data-function
    ))|#
...
(defun get-scale&subscale-datalists (scale-name
                           &key (scaledata-list *CSQ-scaledata-list))
...
;;
;;ddd
(defun find-single-selection-question-data (qvar  &key (all-questdata-list *CSQ-data-list))
...
;;WORKS FOR OLD FORMAT OF *CSQ-SCALEDATA-LIST
#|(defun get-scale-datalist (scale-name
                           &key (datalist *CSQ-scaledata-list))
...
;;  (get-scale-datalist 'INTSS7COLLAB) ;;; :datalist *CSQ-data-list)
...
(defun get-scaledata-qvars (scaledata-list)
  "In U-data-results-functions, find the scale qvar list (following :qv key) in the scaledata-list in all-scaledata-lists = *CSQ-scaledata-list"
...
(defun get-scaledata-subscales (scaledata-list)
  "In U-data-results-functions, find the subscale list or nil (following :ss key) in the scaledata-list in all-scaledata-lists = *CSQ-scaledata-list"
...
;;MAKE-PC-ITEMS-RATINGS-LISTS
;;2018
;;ddd
;;NAME
;;2018
;;ddd
(defun make-pc-items-ratings-lists (csq-data-list) ;;  &key  )
  "U-data-results-functions.  For making inputs to manage-value-rank-frames from  *csq-data-list  RETURNS (values  pc-items-ratings-lists N)"
  (let
      ((pc-items-ratings-lists)
       (pcsym-elm-lists (get-key-value :pcsym-elm-lists csq-data-list))
       (n 0)
       )
    ;;(break "1")
    (when pcsym-elm-lists
    (loop
     for elmlist in pcsym-elm-lists
     do
     (let*
         ((valist (fourth (second elmlist)))
          (name (format nil "~A" (second valist)))
          (value-str (fifth valist))
          )
     (incf n)
     (setf pc-items-ratings-lists (append pc-items-ratings-lists
                                          (list (list name value-str))))
     ;;end let,loop, when
     )))
    ;;(break)
    (values  pc-items-ratings-lists N)
    ;;end let, make-pc-items-ratings-lists
    ))
;;TEST
;; (make-pc-items-ratings-lists *file-csq-data-list)
