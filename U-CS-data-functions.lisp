;; ************************ U-CS-data-functions.lisp **************************
;;
;;THIS FILE IS DEPRECIATED.  FUNCTIONS MOVED TO U-CS.LISP
...
(defun get-category-qvar-names (category &key (all-category-lists  (eval *cur-qvar-lists)))
  ;;was *shaq-question-variable-lists
  "In U-data-functions.lisp, RETURNS (values category-qvar-names = a list of all the qvar names for qvar category AND category)."
  (let 
      ((category-qvar-lists)
       (category-qvar-names)
       )
    ...
    (values category-qvar-names category)
    ;;end let, get-category-qvar-names
    ))
;;TEST
;;  (get-category-qvar-names 'pce-people)  
;;  works= ("mother" "father" "best-m-friend" "best-f-friend")   PCE-PEOPLE
...
(defun get-question-text (qvar &optional (category T))
  ...
  (let*
      ((q-text-name (format nil "~AQ" qvar))
       ...
       (q-var-list 
        (get-keyvalue-in-nested-list  (list (list category 0) (list q-text-sym 0))
    ...
    (cond
     (q-var-list    
      (setf q-text-list (second q-var-list)
            ...
            text-input-box-text (fourth q-var-list))
      ...
      (when (symbolp q-title)
        (setf q-instr (eval q-title)))
      (when (symbolp q-instr)
        (setf q-instr (eval q-instr)))
      (when (symbolp text-input-box-text)
        (setf text-input-box-text (eval text-input-box-text)))
      )
     (t (setf q-text-list '("NO QUESTION TEXT FOUND"))))

    (values q-text-sym q-text-list q-title q-instr text-input-box-text)  
    ;;end get-question-text
    ))
;;test
;;  (get-question-text  "motherq")
